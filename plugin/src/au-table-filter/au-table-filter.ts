...
export class AuTableFilter {
    ...
    @bindable public filters: Array<AuFilter>;
    ...
    public should_add_filter(filter: AuFilter, column: number): boolean {
        return filter.apply_to_columns.some(x => x == column);
    }

    public async select_filter(event: any, filter: AuFilter, column: number): Promise<void> {
        ...
        if (value) {
            ...
            let response = await this.on_filter(this.parameters) as AuTableResponse;
    ...
    public is_selected_filter(filter: AuFilter, column: number): boolean {
