...
export class AuTablePagination {
    ...
    @bindable public page_size: number;
    ...
    @bindable({ changeHandler: 'initial_pagination_load' }) public total_records: string;
    @bindable({ defaultBindingMode: bindingMode.twoWay, changeHandler: 'initial_pagination_load' }) public current_page: number;
    @bindable({ defaultBindingMode: bindingMode.twoWay }) public table_data: number;
    ...
    private initial_pagination_load(): void {
        if (!this.current_page || !this.total_records) return;
        this.refreshing = true;
        this.total_pages = Math.ceil(parseInt(this.total_records) / this.page_size);
    ...
    public async next_page(): Promise<void> {
        ...
        this.current_page++;
        this.table_data = await this.on_next_page(this.skip, this.page_size, this.current_page);
        this.refreshing = false;
    }
    ...
    public async previous_page(): Promise<void> {
        ...
        this.skip -= this.page_size;
        this.current_page--;
        this.table_data = await this.on_previous_page(this.skip, this.page_size, this.current_page);
        this.refreshing = false;
    }
